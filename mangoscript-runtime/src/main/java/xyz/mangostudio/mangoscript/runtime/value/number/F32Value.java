...
public record F32Value(float value) implements NumberValue {
	@Override
	public PrimitiveType getType() { return PrimitiveType.F32; }
	...
	public Value add(Value another) {
		...
		if (another instanceof F32Value a) return new F32Value(value + a.value);
	...
	public Value lessThan(Value another) {
		if (another instanceof F32Value a) return new I32Value(value < a.value ? 1 : 0);
		NumberValue[] a = NumberValue.castForMathOperators(this, another);
